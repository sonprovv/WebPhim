...
export default async function MovieListPage({
  ...
  try {
    ...
    const [moviesResponse, categoriesResponse, genresResponse, countriesResponse] = await Promise.allSettled([
      (async () => {
        try {
          ...
          console.error(`Error in getMoviesByCategory for ${type}:`, error);
          throw error;
        }
      })(),
      api.getCategories(),
    ...
    console.error('Error in MovieListPage:', error);
    return (
      <div className="min-h-screen bg-gray-900 text-white">
        <Header />
        <main className="container mx-auto px-4 py-8">
          <div className="text-center py-12">
            ...
            <p className="text-gray-400 mb-6">
              {error instanceof Error ? error.message : 'Không thể tải danh sách phim. Vui lòng thử lại sau.'}
            </p>
